const imageSets = [
  ...
  window.onload = function () {
    ...
    const currentCard = imageSets.filter(item => item.id === currentId);
     console.log(currentCard);

     if (currentCard.length > 0) {
        const card = `<div class="current-card">
          <div class="current-card-info">
            <h1>${currentCard[0].title}</h1>
            <h1>${currentCard[0].name}</h1>
            <img src=${currentCard[0].images[0]}>
          </div>`;
        
        cardContainer.innerHTML = card;
      } else {
        cardContainer.innerHTML = "Image set not found.";
      }
