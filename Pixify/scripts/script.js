const imageSets = [
  ...
  const displayCards = (list) => {
    ...
    const allCards = list.map((item) => {
        return `<a class="photoCard" href="details.html?id=${item.id}">
        <img src=${item.images[0]} class="pic" />
        <div class="cardInfo">
  ...
  const submitBtn = document.querySelector("#subForm");
  ...
  submitBtn.addEventListener("click", (e) => {
    ...
    console.log(name, title, url1);

    const newCard = document.createElement('div');
    newCard.innerHTML = `<img src=${url1} class="newPic">
    <div class="cardInfo">
